...
@section('content')
    ...
    @foreach ($images as $image)
      <div class="outlined-box">
        ...
        <i class="fa fa-trash" id="{{$image->id}}" data-toggle="modal" data-target="#confirmExclude" aria-hidden="true"></i>
      </div>
      <br>
      <form action="/admin/products/excludeimage" id="excludeForm{{$image->id}}" method="POST">
        @csrf
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '#excludeConfirmed', function() {
          $.ajax({
            type: "POST",
            url: '/admin/products/excludeimage',
            data: $(`excludeForm${currentId}`).serialize(), // serializes the form's elements.
